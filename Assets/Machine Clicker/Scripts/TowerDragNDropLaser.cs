...
public class TowerDragNDropLaser : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(previewTower != null && previewTower.GetComponentInChildren<TowerRay>())
        {
        ...
        else
        {
            ...
            if(hit.collider != null && hit.collider.CompareTag("TowerLaser_Btn"))
            {
                towerBtn.Show();

                if(UnityEngine.Input.GetMouseButtonDown(0) && hit.collider.CompareTag("TowerLaser_Btn"))
                {
                    ...
                    if(towerBtn.defaultPrice <= EconomyManager.instance.CheckTotalIncome())
                    {
                        ...
                        previewTower = Instantiate(EntityManager.instance.laserTower);
                        previewTower.GetComponentInChildren<CircleCollider2D>().enabled = false; // disable collisions while previewing
                        if(previewTower.GetComponentInChildren<TowerRay>()) previewTower.GetComponentInChildren<TowerRay>().enabled = false;
                        isDraggin = true;
                    }
                    else
                    {
    ...
    void TryPlaceTower()
    {
        ...
        // Place tower
        if(previewTower.GetComponentInChildren<TowerRay>()) previewTower.GetComponentInChildren<TowerRay>().enabled = true;
