...
public class TowerDragNDropRocket : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(previewTower != null && previewTower.GetComponentInChildren<Tower_Rocket>())
        {
        ...
        else
        {
            ...
            if(hit.collider != null && hit.collider.CompareTag("TowerRocket_Btn"))
            {

                towerBtn.Show();

                if(UnityEngine.Input.GetMouseButtonDown(0) && hit.collider.CompareTag("TowerRocket_Btn"))
                {
                    if(towerBtn.defaultPrice <= EconomyManager.instance.CheckTotalIncome())
                    {
                        ...
                        previewTower = Instantiate(EntityManager.instance.rocketTower);
                        previewTower.GetComponentInChildren<CircleCollider2D>().enabled = false; // disable collisions while previewing
                        if(previewTower.GetComponentInChildren<Tower_Rocket>()) previewTower.GetComponentInChildren<Tower_Rocket>().enabled = false;
                        isDraggin = true;
                    }
                    else
                    {
    ...
    void TryPlaceTower()
    {
        ...
        // Place tower
        if(previewTower.GetComponentInChildren<Tower_Rocket>()) previewTower.GetComponentInChildren<Tower_Rocket>().enabled = true;
